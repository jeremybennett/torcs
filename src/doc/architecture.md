...
loaded based on the configuration during runtime, so if you have an alternative 
...
cars at once. The major function is "rbDrive", which takes as argument an index 
of the car to drive with the call, a "tCarElt *" containing information about 
the car and a "tSituation *" containing the situation. As result the call fills 
...
implementations, you can find more robots on The TORCS Racing Board (you can 
...
src/libs/raceengineclient/racestate.cpp, ReStateManage, case RE_STATE_RACE. In 
...
ReUpdate is located in src/libs/raceengineclient/raceengine.cpp. It has four 
very different operating modes:

RM_DISP_MODE_NORMAL:
This mode is the most suitable solution for interactive 
simulations, in takes into account simulation time and real time. It basically 
works like this: Progress the simulation until simulation time has catched up 
with real time, then render a frame. Now this has some consequences, because 
...
operating environment hooks up some resources, it might be that TORCS sleeps 
for a moment, and after that nap it has to catch up. So a consequence of 
this mode is that even if you run the exact same simulation in simulation time, 
...
long to appear. So if you are looking to simulate a video source with exactly 
one frame every 0.033 s RM_DISP_MODE_CAPTURE might be a better solution. 
RM_DISP_MODE_NORMAL mode has two more features: If the simulation lags behind a 
lot and is not able to chatch up, it spits out at least a frame on every 2000th 
timestep, this is required to keep the GUI interactive (processing the event 
queue). There is as well a time warp (up to 128 times real time) and slowmotion 
...
between runs from and back to RM_DISP_MODE_NORMAL to inverstigate problems.

RM_DISP_MODE_CAPTURE:
This mode is used to capture frames at exact points in simulation time (e.g. 
every 0.033s). The time is not synchronized with real time, so the simulation 
might be faster or slower than real time, depending on the load you generate. 
Time warp and slowmotion modes are available as well. This would be with some 
...
As you can see in all these modes, ReOneStep in 
src/libs/raceengineclient/raceengine.cpp is then called at some point, this 
actually executes the simulation timestep. Here is a good point to set a 
breakpoint to investigate the call stack when you are new to the TORCS code
base.
...
nonlearning, perfect controlled, hardcoded robot would drive lap by lap the 
same lap (and laptime), would you? But that is not the case. Lets assume that 
we hit on our Nth lap accidently exactly the starting line with the car center, 
and let us draw a point on every simulated timestep on the track. Now when we 
...
and the simulation timestep is 0.002s the simulation is expected to run stable 
up to 1 Million Days simulation time. The longest known performed experiments 
are 100000km races in command line mode.
