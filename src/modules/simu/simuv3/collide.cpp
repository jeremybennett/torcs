...
SimCarCollideZ(tCar *car)
{
	...
	if ((wheel->state & SIM_SUSP_COMP)
		...
		if (dotProd < 0) {
		    if (dotProd <-5.0) {
				car->collision |= 16;
				wheel->rotational_damage_x -= dotProd*0.01*rand()/(RAND_MAX+1.0);
				wheel->rotational_damage_z -= dotProd*0.01*rand()/(RAND_MAX+1.0);
				wheel->bent_damage_x += 0.1*rand()/(RAND_MAX+1.0)-0.05;
				wheel->bent_damage_z += 0.1*rand()/(RAND_MAX+1.0)-0.05;
		...
		if (0) {//rel_normal.z <= 0) {
		    ...
		    forces.x += 0.1*SIGN(car->corner[i].vel.ax) * dotProd;
		    forces.y += 0.1*SIGN(car->corner[i].vel.ay) * dotProd;
...
SimCarCollideXYScene(tCar *car)
{
	...
	if (dotProd < 0) {
		...
		force[2] = invK * (rand()/(RAND_MAX+1.0)-0.5)*0.1;
		...
		if (sgLengthVec3 (force) > 0.01) {
			...
			poc[0] = corner->pos.x;
			poc[1] = corner->pos.y;
			poc[2] = (rand()/(RAND_MAX+1.0)-0.5)*2.0;
			t3Dd angles;
			t3Dd original;
			t3Dd updated;
			angles.x = car->DynGCg.pos.ax;
