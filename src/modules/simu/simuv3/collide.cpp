...
SimCarCollideZ(tCar *car)
{
	...
	if ((wheel->state & SIM_SUSP_COMP)
		...
		if (dotProd < 0) {
		    if (dotProd <-5.0) {
				car->collision |= 16;
				wheel->rotational_damage_x -= dotProd*0.01*rand()/RAND_MAX;
				wheel->rotational_damage_z -= dotProd*0.01*rand()/RAND_MAX;
				wheel->bent_damage_x += 0.1*rand()/RAND_MAX-0.05;
				wheel->bent_damage_z += 0.1*rand()/RAND_MAX-0.05;
...
SimCarCollideXYScene(tCar *car)
{
	...
	if (dotProd < 0) {
		...
		force[2] = invK * (rand()/RAND_MAX-0.5)*0.1;
