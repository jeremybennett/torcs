...
static void
CreateSegRing3(void *TrackHandle, char *section, tTrackSeg **pRoot, tdble *pLength, int *pNseg, tTrackSeg *start, tTrackSeg *end, int ext)
{
    int		j;
...
#define MAX_TMP_INTS	256
	...
	/* Turn Marks */
	if (ext) {
	    char *marks = GfParmGetCurStr(TrackHandle, path, TRK_ATT_MARKS, "");
	    ind = 0;
	    if (strlen(marks)) {
		char *s = strtok(marks, ";");
...
ReadTrack3(tTrack *theTrack, void *TrackHandle, tRoadCam **camList, int ext)
{
	...
	if (strlen(segName) != 0) {
		...
		pitExitSeg = pitExitSeg->prev; /* go back in the list */
	    }
	    if (!found) {
		pitExitSeg = NULL;
	    }
	...
	do {
		...
		*camList = curCam;
	    ...
	    segName = GfParmGetCurStr(TrackHandle, path, TRK_ATT_SEGMENT, "");
	    if (strlen(segName) == 0) {
		GfTrace2("Bad Track Definition: in Camera %s %s is missing\n", curCam->name, TRK_ATT_SEGMENT);
		exit(1);
	    }
	    ...
	    segName = GfParmGetCurStr(TrackHandle, path, TRK_ATT_CAM_FOV, "");
	    if (strlen(segName) == 0) {
	    ...
	    segName = GfParmGetCurStr(TrackHandle, path, TRK_ATT_CAM_FOVE, "");
	    if (strlen(segName) == 0) {
