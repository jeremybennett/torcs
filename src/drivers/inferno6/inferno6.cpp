/***************************************************************************
    ...
    file        : inferno5.cpp
    created     : Mon Nov 11 12:00:29 CET 2002
 ...
 */
extern "C" int
inferno6(tModInfo *modInfo)
{
    modInfo->name    = "Inferno 6";	/* name of the module (short) */
...
static void initTrack(int index, tTrack* track, void *carHandle, void **carParmHandle, tSituation *s)
{
    ...
    sprintf(ParamNames, "drivers/inferno6/tracksdata/car_%s", str);
    *carParmHandle = GfParmReadFile(ParamNames, GFPARM_RMODE_STD);
    if (*carParmHandle == NULL) {
	sprintf(ParamNames, "drivers/inferno6/car1.xml");
	*carParmHandle = GfParmReadFile(ParamNames, GFPARM_RMODE_STD | GFPARM_RMODE_CREAT);
	GfOut("drivers/inferno6/car1.xml Loaded\n");
    } else {
	GfOut("%s Loaded\n", ParamNames);
    }
...
/*     Gmax = Gmax * GfParmGetNum(*carParmHandle, SECT_CAR, PRM_MASS, (char*)NULL, 1000.0); */
    ...
    sprintf(buf, "drivers/inferno6/tracksdata/%s", str);
...
#define AMARG 0.6
	...
	if (s->_raceType == RM_TYPE_PRACTICE) {
	    if (lap == 1) {
		RtTelemStartMonitoring("Inferno6");
	    }
