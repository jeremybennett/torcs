 ...
 */
extern "C" int
inferno4(tModInfo *modInfo)
{
    modInfo->name    = "Inferno 4";	/* name of the module (short) */
...
static void initTrack(int index, tTrack* track, void *carHandle, void **carParmHandle, tSituation *s)
{
    ...
    sprintf(ParamNames, "drivers/inferno4/tracksdata/car_%s", str);
    *carParmHandle = GfParmReadFile(ParamNames, GFPARM_RMODE_STD);
    if (*carParmHandle == NULL) {
	sprintf(ParamNames, "drivers/inferno4/car1.xml");
	*carParmHandle = GfParmReadFile(ParamNames, GFPARM_RMODE_STD | GFPARM_RMODE_CREAT);
	GfOut("drivers/inferno4/car1.xml Loaded\n");
    } else {
	GfOut("%s Loaded\n", ParamNames);
    }
...
/*     Gmax = Gmax * GfParmGetNum(*carParmHandle, SECT_CAR, PRM_MASS, (char*)NULL, 1000.0); */
    ...
    sprintf(buf, "drivers/inferno4/tracksdata/%s", str);
