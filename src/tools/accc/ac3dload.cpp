...
int doNumvert(char *Line, ob_t *object, mat_t *material)
{
  ...
  /*memset(object->next->vertex,'\0',sizeof(point_t )*object->next->numvert);*/
  numvertex=0;
  numvertFound=1;
  return (0);
}
...
int doNumsurf(char *Line, ob_t *object, mat_t *material)
{
  ...
  /*object->next->vertexarray=(int *)
    malloc(sizeof(int)*object->next->numsurf*3);
    memset(object->next->vertexarray,'\0',sizeof(int
    )*object->next->numsurf*3);*/
  numvertFound=0;
  return (0);
}
      ...
      while (1)
        {
          if (stricmp("END",verbTab[i].verb)==0)
            break;
          if
	    (strnicmp(Line,verbTab[i].verb,strlen(verbTab[i].verb))==0)
            {
...
void  saveIn3DSsubObject(ob_t * object,database3ds *db)
{
  ...
  if (object->name==NULL &&  (!stricmp("world",object->type)))
    ...
    if (!stricmp("world",object->name)) {
      return;
    }
  else
    
    if (!stricmp("root",object->name)) {
      if (object->next!=NULL){
   ...
   for (i=0; i<texnum; i++) 
     {
       ...
       InitMaterial3ds(&matr);
       ON_ERROR_RETURN;
       /*strcpy(matr->name, tex[i]);
	 strcpy(matr->texture.map.name, tex[i]);
       */
...
void computeTriNorm(ob_t * object )
{
  ...
  while (tmpob!=NULL) {
	...
	/*
	tmpob->snorm[tmpob->vertexarray[i*3].indice].x+=norm.x;
	tmpob->snorm[tmpob->vertexarray[i*3].indice].y+=norm.y;
	tmpob->snorm[tmpob->vertexarray[i*3].indice].z+=norm.z;

	tmpob->snorm[tmpob->vertexarray[i*3+1].indice].x+=norm.x;
	tmpob->snorm[tmpob->vertexarray[i*3+1].indice].y+=norm.y;
	tmpob->snorm[tmpob->vertexarray[i*3+1].indice].z+=norm.z;

	tmpob->snorm[tmpob->vertexarray[i*3+2].indice].x+=norm.x;
	tmpob->snorm[tmpob->vertexarray[i*3+2].indice].y+=norm.y;
	tmpob->snorm[tmpob->vertexarray[i*3+2].indice].z+=norm.z;
	*/
      }
    for (i=0; i<tmpob->numsurf; i++)
      {
		...
		if( checkMustSmoothVector( &tmpob->norm[i],&tmpob1->norm[j],&tmpob->vertex[i],&tmpob1->vertex[j] ))
		  {
		    ...
		    /*tmpob->snorm[i].x=tmpob1->norm[j].x;
		      ...
		      tmpob1->norm[j].x=tmpob->snorm[i].x;
		      tmpob1->norm[j].y=tmpob->snorm[i].y;
		      tmpob1->norm[j].z=tmpob->snorm[i].z;
		      tmpob->norm[i].x=tmpob1->norm[j].x;
		      tmpob->norm[i].y=tmpob1->norm[j].y;
		      tmpob->norm[i].z=tmpob1->norm[j].z;
		      tmpob->snorm[i].x=tmpob1->norm[j].x;
		      tmpob->snorm[i].y=tmpob1->norm[j].y;
		      tmpob->snorm[i].z=tmpob1->norm[j].z;
		    */
	...
	if (tmpob1!=tmpob )
	  ...
	  for (i=0; i<tmpob->numvert; i++)
	      ...
	      for (j=0; j<tmpob1->numvert; j++)
		{
		  if( checkMustSmoothVector( &tmpob->norm[i],&tmpob1->norm[j],&tmpob->vertex[i],&tmpob1->vertex[j] ))
		    {
		      if (tmpob->snorm[i].x!=tmpob1->snorm[j].x ||
...
void computeSaveOBJ( char * OutputFilename, ob_t * object)
{
  ...
  while (tmat!=NULL)
    {
      /*fprintf(ofile, "MATERIAL %s rgb %lf %lf %lf amb %lf %lf %lf emis %lf %lf %lf spec %lf %lf %lf shi %d trans %d \n",
	     tmat->name,
	     tmat->rgb.r,
	     tmat->rgb.g,
	     tmat->rgb.b,
...
void stripifyOb(ob_t * object,int writeit)
{
  ...
  system(command);
  /*  sprintf(command, "erase %s", filename);
      system(command);*/
  /*stripein = fopen("stripe.objf","r");*/
  strcat(filename, "f");
  ...
  while (fscanf(stripein, "%s", line) != EOF) {
    ...
    default:
      ...
      /*printf("strip point %d\n",StripPoint[CurrentStripPoint]);*/
      CurrentStripPoint++;
      CurrentStripLength++;
      break;
    }
  ...
  /*  for (j=0; j<NumStripPoints; j++)
    if (StripPoint[j] < 0 || StripPoint[j] > (facecur->numtri*3)-1)
      exit(-1);
  */
}
...
void mapTextureEnv(ob_t *object)
{
  ...
  while (tmpob!=NULL) {
	...
	tmpob->textarray1[i*2]= (tmpob->vertex[i].x-x_min)/(x_max-x_min) +(tmpob->snorm[i].x)/2;
	tmpob->textarray1[i*2+1]= ((tmpob->vertex[i].z-z_min)/(z_max-z_min))+(tmpob->snorm[i].z)/2; /* + fabs((tmpob->snorm[i].x)/2);*/
